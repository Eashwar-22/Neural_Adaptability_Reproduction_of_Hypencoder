...
### Hardware Differences (Paper vs. Reproduction)
The results reported in the original paper were obtained by training on **2x NVIDIA A100 GPUs**.
The "Retrained" models provided in this repository validation were trained on **8x NVIDIA H100 GPUs**.

While hyperparameters (learning rate, batch size, etc.) were kept consistent, minor performance differences (e.g., ±1-3% on OOD tasks) are expected due to:
1. **Architecture Optimizations**: H100s (Hopper) handle floating-point precision (TF32/BF16) differently than A100s (Ampere).
2. **Distributed Dynamics**: Scaling from 2 to 8 GPUs affects gradient accumulation and batch normalization statistics slightly.
