...
#SBATCH --output=logs/eval_hype_bge_%j.log

source ~/.bashrc
conda activate hype_env

MODEL_PATH="./checkpoints/hypencoder_teacher_sota"
BASE_OUT_DIR="./outputs/inference/hypencoder_teacher_sota"

export HF_HOME="${HYPENCODER_CACHE:-./cache}"
export HF_DATASETS_CACHE="${HYPENCODER_CACHE:-./cache}"

echo "========================================================"
echo "Evaluating Hypencoder (BGE Teacher SOTA)"
