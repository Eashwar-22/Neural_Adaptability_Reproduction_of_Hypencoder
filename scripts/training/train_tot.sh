...
#SBATCH --output=logs/train_tot_%j.log
...
JOB_DIR="/dev/shm/hype_tot_$SLURM_JOBID"
mkdir -p $JOB_DIR
...
echo "--- Moving config to RAM Disk ($JOB_DIR) ---"

# Copy config to RAM
cp my_configs/hypencoder_tot.yaml $JOB_DIR/config.yaml

# Update config to point to ABSOLUTE data path (just in case)
ABS_DATA_PATH="./data/triples.train_tot.jsonl"
...
echo "Data: $ABS_DATA_PATH"
echo "Launching TOT Training (Hypencoder)..."
