...
# SECURITY NOTE: PyTorch 2.0+ defaults to weights_only=True to prevent arbitrary
# code execution via pickle. We override this for backward compatibility with
# older checkpoints. Only load checkpoints from trusted sources!
_original_torch_load = torch.load
def _safe_torch_load(*args, **kwargs):
    """Load checkpoints with weights_only=False for backward compatibility.
    
    WARNING: This allows loading pickled Python objects, which can execute
    arbitrary code. Only use with trusted checkpoint files.
    """
...
def load_model(model_config: HypencoderModelConfig):
    ...
    if model_config.use_lora:
        ...
        peft_config = LoraConfig(
            ...
            # Target standard BERT modules. 
            # Note: Depending on transformers version/model type, targets might vary.
            # safe list for BERT/RoBERTa
            target_modules=["query", "key", "value", "dense"], 
        )
        
        # Apply to query encoder's transformer
        # Hypencoder structure: self.transformer
        ...
        if model_config.shared_encoder:
            ...
            # If not shared, wrap passage encoder too (it also has .transformer)
...
def load_data(data_config: HypencoderDataConfig):
    ...
    if data_config.training_huggingface_dataset is not None:
        ...
        if os.path.exists(dataset_path):
            ...
            # Using rank checking to avoid write conflicts if possible, though strict sync is hard here.
            # Assuming shared file system and standard accelerate launch.
...
def get_collator(
    ...
    return GeneralDualEncoderCollator(
        ...
        query_dropout_prob=data_config.query_dropout_prob,
    )
...
def train_model(cfg: HypencoderTrainingConfig):
    ...
    # cfg.trainer_config.hf_trainer_config.torch_compile = False
    ...
    # Ensure local_rank is correctly set from env if not present
    ...
    training_args.torch_compile = False # Force disable

    # Force disable saving on each node to prevent race conditions
    ...
    class CustomTrainer(Trainer):
        def _save_checkpoint(self, model, trial, metrics=None):
            # Calculate the final checkpoint directory
            checkpoint_folder = f"{PREFIX_CHECKPOINT_DIR}-{self.state.global_step}"
            run_dir = self.args.output_dir
            output_dir = os.path.join(run_dir, checkpoint_folder)
            
            # 1. Create directory on all ranks and wait
            os.makedirs(output_dir, exist_ok=True)
            if torch.distributed.is_initialized():
                torch.distributed.barrier()
            
            # 2. Save model (directly to final dir, skipping tmp- staging)
            ...
            # 6. Rotate checkpoints (delete old ones)
            if self.args.should_save:
    ...
    trainer.save_model()
    # Also save the state just in case we want to resume exactly from here later (though less critical for final model)
    trainer.save_state()
