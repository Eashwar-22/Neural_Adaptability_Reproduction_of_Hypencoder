...
class GeneralDualEncoderCollator:
    ...
    def __init__(
        ...
        self.query_dropout_prob = query_dropout_prob
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if self.query_dropout_prob > 0:
            new_queries = []
            for f in features:
                tokens = f["query"]["tokenized_content"]
                # Assume standard BERT behavior: tokens[0] is CLS, tokens[-1] is SEP or end
                # Only drop middle tokens
                if len(tokens) > 2:
                    keep_tokens = [tokens[0]]
                    middle_tokens = tokens[1:-1]
                    # Keep if random > prob
                    filtered_middle = [t for t in middle_tokens if random.random() > self.query_dropout_prob]
                    # If we dropped everything, keep at least one random token to avoid empty query
                    if not filtered_middle and middle_tokens:
                        filtered_middle = [random.choice(middle_tokens)]
                    keep_tokens.extend(filtered_middle)
                    keep_tokens.append(tokens[-1])
                    new_queries.append({"input_ids": keep_tokens})
                else:
                    new_queries.append({"input_ids": tokens})
            queries = new_queries
        else:
            queries = [
