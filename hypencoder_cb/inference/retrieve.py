...
class HypencoderRetriever(BaseRetriever):
    ...
    def retrieve(self, query: TextQuery, top_k: int) -> List[Item]:
        ...
        for batch_index, batch_item_embeddings in enumerate(
            ...
            if self.model_type == "text_dual_encoder":
                # Standard Bi-Encoder dot product
                similarity_matrix = torch.matmul(
                    query_model, batch_item_embeddings.transpose(0, 1)
                ).squeeze()
            else:
                # Hypencoder Q-Net forward pass
            ...
            actual_k = min(top_k, similarity_matrix.shape[0])
            values, indices = torch.topk(similarity_matrix, actual_k, dim=0)
